...
class Reactors {
	...
	public function assign($pos=null) {
		if ($pos == null) return false;
		
		$this->_tableX = $this->_table_silos_reactors;
		if ($this->doLocations()) {
			$this->assignReactions($pos);
			$this->_db->msg->addconfirm( 'Reactors assigned' );
		}
	}
	
	public function assignAll($locationID=null) {
		if ($locationID == null) return false;
		
		$this->_tableX = $this->_table_silos_reactors;
		if ($this->doLocations()) {
			$this->assignReactionsAll($locationID);
			$this->_db->msg->addconfirm( 'Reactors assigned' );
		}
	}
	...
	private function doLocations() {
		...
		if ($api = $this->getAPI()) {
			...
			try {
				...
				$str = "SELECT * FROM {$this->_tableX} WHERE corpID='{$this->_corpID}' AND pos IS NULL";
	...
	private function assignReactions($tower) {
		$complex = array(16670,16671,16672,16673,16678,16679,16680,16681,16682,16683,17317);
			
		$res = $this->_db->query("SELECT * FROM {$this->_table_silos_reactors} WHERE pos = {$tower} AND corpID = {$this->_corpID} ORDER BY typeID ASC");
		while( $row = $res->fetch_assoc() ) {
	...
	private function assignReactionsAll($locationID) {
		$complex = array(16670,16671,16672,16673,16678,16679,16680,16681,16682,16683,17317);
			
		$res = $this->_db->query("SELECT * FROM {$this->_table_silos_reactors} WHERE locationID = {$locationID} AND corpID = {$this->_corpID} ORDER BY typeID ASC");
		while( $row = $res->fetch_assoc() ) {
			$react = $this->_db->query("SELECT r.typeID, r.input, r.quantity * IFNULL(IFNULL(da.valueInt, da.valueFloat), 1) as qty
				FROM {$this->_table['invtypereactions']} r 
				LEFT JOIN {$this->_table['dgmtypeattributes']} da ON r.typeID = da.typeID AND da.attributeID = 726
