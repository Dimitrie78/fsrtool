...
$xml = simplexml_load_file('./database/contents.xml');
...
foreach($kb['table'] as $idx => $tbl)
{
	#echo $tbl['name'].'<br>';
	#mkdir('./database/'.$tbl['name']);
	#continue;
	$table = $tbl['name'];
	$files = array();
	$dir = opendir('./database/'.$table);

	$xml = simplexml_load_file('./database/'.$table.'/table.xml');
	$json = json_encode($xml);
	$st = json_decode($json,TRUE);
	#echo '<pre>'; print_r($st); echo '</pre>'; die;
	$struct[$table] = $st['structure'];
	$kb['table'][$idx]['rows'] = $st['rows'];
	$structc++;
	...
	while ($file = readdir($dir))
	{
		...
		elseif (!strpos($file, 'xml')) //with xml it is probably the table definition file...
		{
			$dcnt++;
			$datacnt++;
			$data[$table][] = './database/'.$table.'/'.$file;
			asort($data[$table]);
		}
	}
}
...
if (!empty($_REQUEST['sub']) && $_REQUEST['sub'] == 'data')
{
	...
	if (!$did)
	{
		...
		else
		{
			...
			echo '<a href="?step=5">Next Step --&gt;</a><br/>';
		}
	}
