{if $previous_install}
    <br/><div class="block-header2"><img src="{$previous_image}" border="0" alt=""> The FSRTOOL is already installed!</div>
    The FSRTOOL is already installed. You can either update your tool or erase all data and reinstall.<br/>
	<p><a href="{$update}">Update --&gt;</a></p>
	<p><a href="?step=2&erase=1">Permanently erase database --&gt;</a></p>

    <p>If you do not wish to run the installation then remove the /install directory from your FSRTOOL installation to ensure it isn't run again.
    </p>
{/if}
...
{if $gd_exists}<b>GD is available.</b><br/>
    ...
    {/if}
{else}
    <b>GD is NOT available.</b><br/>The Fsrtool will be unable to output character portraits, corporation logos or signatures. Please speak with your system administrator to install GD 2.0 or higher.<br/>
    <br/>
    You can continue with the installation, but the Fsrtool might not run correctly.<br/>
{/if}
...
{if !$conf_exists}Please create the file "config.ini" and make it writeable by the webserver.<br/>
{elseif $conf_conditional}
    I cannot write into "../dbconfig.ini", you need to fix that for me before you can continue.<br/>
    Please issue a "chmod 777 ../config" on the commandline inside of this directory<br/>
{else}The config file "../dbconfig.ini" is there and writeable, excellent!<br/>
{/if}
...
<br/>
<b>Let's try cURL now; it's used for some sections of the fsrtool:</b><br/>
{if $conn_curl_exists}
