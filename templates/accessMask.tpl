...
<a href="https://support.eveonline.com/api/Key/CreatePredefined/42336264" target="_blank">Get valid API</a>
...
{foreach from=$list item=group}
	...
	{if $key !== 'des'}
	  {if $type.accessMask & $mask}
		{if isset($error) && in_array($type.name, $error)}<tr bgcolor="#FF0000"><td><img src="icons/tick.png" width="12" height="12" /></td><td>{$type.name}</td><td>{$type.description}</td></tr>
		{else}<tr><td><img src="icons/tick.png" width="12" height="12" /></td><td>{$type.name}</td><td>{$type.description}</td></tr>{/if}
	  {else}
		{if isset($error) && in_array($type.name, $error)}<tr bgcolor="#FF0000"><td><img src="icons/cross.png" width="12" height="12" /></td><td>{$type.name}</td><td>{$type.description}</td></tr>
